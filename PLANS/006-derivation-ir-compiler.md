# 006 - Derivation IR Compiler

## Task
Implement derivation compiler from visual builder model to DeriveIR covering value and inputProps/customData override targets.
...
- `apps/site/src/features/plugin-builder/domain/ir/derivation-ir-compiler.test.ts`

## Read-only Context
- `packages/supersurkhet-sdk/src/index.ts`
- `apps/site/src/lib/zod/with-derivations.ts`
- `apps/site/src/components/ui/autoform/derive.ts`
...
## Definition Of Done
- Compiler supports all locked expression operators and typed references.
- Target path support includes value inputProps and customData branches.
- Tests validate compile output and execution compatibility assumptions.

## Verification
- `pnpm --filter supersurkhet test -- derivation-ir-compiler.test.ts`
