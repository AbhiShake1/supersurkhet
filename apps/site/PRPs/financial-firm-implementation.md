---
document_type: prp
title: Financial Firm Implementation
version: 1.0
author: AI IDE Agent
date: 2025-08-23
status: draft
...
# PRP: Financial Firm Implementation

## Goal

### Feature Goal
Create a comprehensive system for financial firm businesses with production-ready schemas, custom admin components for client portfolio management, and beautiful client pages for financial product browsing and appointment booking.

### Deliverable
A complete implementation including:
1. Production-ready Zod schema for financial firm businesses with all necessary fields
2. Custom admin component for client portfolio management that renders as a tab in the AutoAdmin panel
3. Beautiful, modern client page for financial product browsing and appointment booking
...
- The financialFirm schema is properly defined with appropriate fields for financial services, products, and advisors
- The admin component renders correctly in the AutoAdmin panel with functionality for managing client portfolios
- The client page displays financial products and services in an intuitive and visually appealing way with appointment booking capability
...
existing_codebase_files:
  - file: src/lib/schema.ts
    ...
    relevance: This is where the financialFirm schema will be defined, following the existing pattern
    
  - file: src/components/ui/admin/hotel-management.tsx
    purpose: Example of a custom admin component for hotel schema
    relevance: This pattern will be replicated for the financial firm admin component
  ...
  - file: src/components/pages/hotel/hotel-client-page.tsx
    ...
    relevance: This pattern will be replicated for the financial firm client page with appropriate variations
  ...
  - file: src/routes/$businessName/index.tsx
    ...
    relevance: This file will need to be updated to handle the financial firm business type
...
2. A businessType enum that includes "financial_firm" 
3. A basic financialFirm schema defined in featureSchema but without a custom admin component
...
The problem identified is that we have a basic schema defined but no custom admin component or client page for financial firm businesses.
...
```yaml
task_1_enhance_financial_firm_schema:
  title: Enhance Financial Firm Schema
  description: Add comprehensive fields to the existing financial firm schema
  files:
    - src/lib/schema.ts
  details: |
    - Enhance the existing financialFirmSchema with additional fields:
...
```yaml
task_2_create_financial_firm_admin_component:
  ...
  files:
    ...
    - src/lib/schema.ts
  details: |
    - Create FinancialFirmManagement component following the HotelManagement pattern
    - Include sections for:
      * Client portfolio management
      * Product offering management
      * Advisor assignment and management
...
```yaml
task_3_create_financial_firm_client_page:
  ...
  details: |
    - Create FinancialFirmClientPage component following the HotelClientPage pattern
    - Include sections for:
      * Hero section with trust-building elements
      * Financial services offered with descriptions
      * Financial products with details
...
task_4_update_autoadmin_integration:
  ...
  files:
    ...
    - src/lib/schema.ts
  details: |
    - Verify that the component loading mechanism works for the financialFirm schema
...
task_5_update_business_routing:
  ...
  description: Update the business route to handle the financial firm business type
  files:
    - src/routes/$businessName/index.tsx
  details: |
    - Add case for "financial_firm" in the switch statement
    - Ensure proper fallback to generic client page if needed
    - Test business type routing

task_6_implement_data_validation:
  title: Implement Data Validation
  description: Add proper validation and error handling for the financial firm schema
  files:
    - src/lib/schema.ts
  details: |
    - Add custom validation where needed for financial firm specific requirements
...
pnpm biome format --write src/lib/schema.ts src/components/ui/admin/financial-firm-management.tsx src/components/pages/financial-firm/financial-firm-client-page.tsx

# Run Biome linter to catch potential issues
pnpm biome lint src/lib/schema.ts src/components/ui/admin/financial-firm-management.tsx src/components/pages/financial-firm/financial-firm-client-page.tsx
```
...
# pnpm test src/components/ui/admin/financial-firm-management.test.tsx
# pnpm test src/components/pages/financial-firm/financial-firm-client-page.test.tsx
```
...
# Manual checks:
# 1. The financialFirm schema is properly defined and exported
# 2. Admin component renders correctly in AutoAdmin panel
# 3. Client page renders correctly for financial firm business type
...
6. Ensure financial data is properly secured and encrypted
