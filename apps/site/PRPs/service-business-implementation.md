---
document_type: prp
title: Service Business Implementation
version: 1.0
author: AI IDE Agent
date: 2025-08-23
status: draft
...
# PRP: Service Business Implementation

## Goal

### Feature Goal
Create a comprehensive system for service businesses with production-ready schemas, custom admin components for appointment management, and beautiful client pages for service booking.

### Deliverable
A complete implementation including:
1. Production-ready Zod schema for service businesses with all necessary fields
2. Custom admin component for appointment management that renders as a tab in the AutoAdmin panel
3. Beautiful, modern client page for service browsing and appointment booking
...
- The service schema is properly enhanced with appropriate fields for service management
- The admin component renders correctly in the AutoAdmin panel with functionality for managing appointments
- The client page displays services in an intuitive and visually appealing way with appointment booking capability
...
existing_codebase_files:
  - file: src/lib/schema.ts
    ...
    relevance: This is where the service schema will be enhanced, following the existing pattern
    
  - file: src/components/ui/admin/hotel-management.tsx
    purpose: Example of a custom admin component for hotel schema
    relevance: This pattern will be replicated for the service admin component
  ...
  - file: src/components/pages/hotel/hotel-client-page.tsx
    ...
    relevance: This pattern will be replicated for the service client page with appropriate variations
  ...
  - file: src/routes/$businessName/index.tsx
    ...
    relevance: This file will need to be updated to handle the service business type
...
2. A businessType enum that includes "service" 
3. A serviceSchema that can be enhanced for service management
...
The problem identified is that we need to enhance the existing service schema and create custom admin/client components for service businesses.
...
```yaml
task_1_enhance_service_schema:
  title: Enhance Service Schema
  description: Add comprehensive fields to the existing service schema for service management
  files:
    - src/lib/schema.ts
  details: |
    - Enhance the existing serviceSchema with additional fields for service businesses:
      * serviceCategories (record of category names to boolean)
      * appointmentSystem (boolean) - Whether appointments are required
      * walkInAllowed (boolean) - Whether walk-ins are accepted
      ...
      * prerequisites (string) - Any requirements before service
...
```yaml
task_2_create_service_admin_component:
  title: Create Service Admin Component
  description: Create a custom admin component for service management
  files:
    - src/components/ui/admin/service-management.tsx
    - src/lib/schema.ts
  details: |
    - Create ServiceManagement component following the HotelManagement pattern
    ...
    - Register component in serviceSchema components function
...
```yaml
task_3_create_service_client_page:
  title: Create Service Client Page
  description: Create a beautiful client page for service businesses
  files:
    - src/components/pages/service/service-client-page.tsx
    - src/routes/$businessName/index.tsx
  details: |
    - Create ServiceClientPage component following the HotelClientPage pattern
    - Include sections for:
      * Hero section with service offerings and promotions
      * Service catalog with filtering and sorting
      * Appointment booking system with calendar integration
      * Service provider profiles
      * Customer reviews and ratings
      * Service details and prerequisites
      * Location and directions
      * Contact information
    - Implement responsive design for all screen sizes
    - Add smooth animations and transitions
    - Ensure dark mode compatibility
    - Update the business route to handle service business type
```
...
task_4_update_autoadmin_integration:
  ...
  files:
    ...
    - src/lib/schema.ts
  details: |
    - Verify that the component loading mechanism works for the service schema
...
task_5_update_business_routing:
  ...
  description: Update the business route to handle the service business type
  files:
    - src/routes/$businessName/index.tsx
  details: |
    - Add case for "service" in the switch statement
    - Ensure proper fallback to generic client page if needed
    - Test business type routing

task_6_implement_data_validation:
  title: Implement Data Validation
  description: Add proper validation and error handling for the service schema
  files:
    - src/lib/schema.ts
  details: |
    - Add custom validation where needed for service specific requirements
...
pnpm biome format --write src/lib/schema.ts src/components/ui/admin/service-management.tsx src/components/pages/service/service-client-page.tsx

# Run Biome linter to catch potential issues
pnpm biome lint src/lib/schema.ts src/components/ui/admin/service-management.tsx src/components/pages/service/service-client-page.tsx
```
...
# pnpm test src/components/ui/admin/service-management.test.tsx
# pnpm test src/components/pages/service/service-client-page.test.tsx
```
...
# Manual checks:
# 1. The service schema is properly enhanced and exported
# 2. Admin component renders correctly in AutoAdmin panel
# 3. Client page renders correctly for service business type
...
6. Ensure appointment data is properly secured
...
9. Test appointment booking and calendar functionality
