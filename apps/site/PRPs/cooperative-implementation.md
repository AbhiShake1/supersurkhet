---
document_type: prp
title: Cooperative Implementation
version: 1.0
author: AI IDE Agent
date: 2025-08-23
status: draft
...
# PRP: Cooperative Implementation

## Goal

### Feature Goal
Create a comprehensive system for cooperatives with production-ready schemas, custom admin components for member management and financial operations, and beautiful client pages for member services and financial information.

### Deliverable
A complete implementation including:
1. Production-ready Zod schema for cooperatives with all necessary fields
2. Custom admin component for member management and financial operations that renders as a tab in the AutoAdmin panel
3. Beautiful, modern client page for member services and financial information
...
- The coOpMemberProfile schema is properly enhanced with appropriate fields for cooperatives
- The admin component renders correctly in the AutoAdmin panel with functionality for managing members and financial operations
- The client page displays member services and financial information in an intuitive and visually appealing way
...
existing_codebase_files:
  - file: src/lib/schema.ts
    ...
    relevance: This is where the coOpMemberProfile schema will be enhanced, following the existing pattern
    
  - file: src/components/ui/admin/hotel-management.tsx
    purpose: Example of a custom admin component for hotel schema
    relevance: This pattern will be replicated for the cooperative admin component
  ...
  - file: src/components/pages/hotel/hotel-client-page.tsx
    ...
    relevance: This pattern will be replicated for the cooperative client page with appropriate variations
  ...
  - file: src/routes/$businessName/index.tsx
    ...
    relevance: This file will need to be updated to handle the cooperative business type
...
2. A businessType enum that includes "cooperative" 
3. A coOpMemberProfileSchema that can be enhanced for cooperatives
...
The problem identified is that we need to enhance the existing coOpMemberProfile schema and create custom admin/client components for cooperatives.
...
```yaml
task_1_enhance_coop_member_profile_schema:
  title: Enhance Cooperative Member Profile Schema
  description: Add comprehensive fields to the existing cooperative member profile schema
  files:
    - src/lib/schema.ts
  details: |
    - Enhance the existing coOpMemberProfileSchema with additional fields for cooperatives:
      * membershipNumber (string) - Official membership number
      * joinDate (string) - Date member joined the cooperative
      * memberType (enum) - Type of membership (regular, associate, honorary)
      * sharesOwned (number) - Number of shares owned by member
      * votingRights (boolean) - Whether member has voting rights
      * contactInfo (object with phone, email, address)
      * emergencyContact (object with emergency contact details)
      * nomineeInfo (object with nominee details)
      * financialRecords (record of transaction IDs to details)
      * meetingAttendance (record of meeting dates to attendance status)
      * committeeMemberships (record of committee names to boolean)
      * loanHistory (record of loan IDs to details)
...
```yaml
task_2_create_cooperative_admin_component:
  title: Create Cooperative Admin Component
  description: Create a custom admin component for cooperative management
  files:
    - src/components/ui/admin/cooperative-management.tsx
    - src/lib/schema.ts
  details: |
    - Create CooperativeManagement component following the HotelManagement pattern
    ...
    - Register component in coOpMemberProfileSchema components function
...
```yaml
task_3_create_cooperative_client_page:
  title: Create Cooperative Client Page
  description: Create a beautiful client page for cooperatives
  files:
    - src/components/pages/cooperative/cooperative-client-page.tsx
    - src/routes/$businessName/index.tsx
  details: |
    - Create CooperativeClientPage component following the HotelClientPage pattern
    - Include sections for:
      * Hero section with cooperative information and values
    ...
    - Update the business route to handle cooperative business type
```
...
task_4_update_autoadmin_integration:
  ...
  files:
    ...
    - src/lib/schema.ts
  details: |
    - Verify that the component loading mechanism works for the coOpMemberProfile schema
...
task_5_update_business_routing:
  ...
  description: Update the business route to handle the cooperative business type
  files:
    - src/routes/$businessName/index.tsx
  details: |
    - Add case for "cooperative" in the switch statement
    - Ensure proper fallback to generic client page if needed
    - Test business type routing

task_6_implement_data_validation:
  title: Implement Data Validation
  description: Add proper validation and error handling for the cooperative schema
  files:
    - src/lib/schema.ts
  details: |
    - Add custom validation where needed for cooperative specific requirements
...
task_7_performance_optimization:
  ...
  details: |
    ...
    - Optimize financial data reporting
...
pnpm biome format --write src/lib/schema.ts src/components/ui/admin/cooperative-management.tsx src/components/pages/cooperative/cooperative-client-page.tsx

# Run Biome linter to catch potential issues
pnpm biome lint src/lib/schema.ts src/components/ui/admin/cooperative-management.tsx src/components/pages/cooperative/cooperative-client-page.tsx
```
...
# pnpm test src/components/ui/admin/cooperative-management.test.tsx
# pnpm test src/components/pages/cooperative/cooperative-client-page.test.tsx
```
...
# Manual checks:
# 1. The coOpMemberProfile schema is properly enhanced and exported
# 2. Admin component renders correctly in AutoAdmin panel
# 3. Client page renders correctly for cooperative business type
...
6. Ensure member financial data is properly secured
7. Implement proper data encryption for sensitive information
8. Ensure compliance with financial data privacy regulations
...
9. Test financial transaction and reporting functionality
...
6. Monitor financial transaction processing
...
9. Optimize financial data reporting performance