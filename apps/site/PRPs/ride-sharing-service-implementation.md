---
document_type: prp
title: Ride Sharing Service Implementation
version: 1.0
author: AI IDE Agent
date: 2025-08-23
status: draft
...
# PRP: Ride Sharing Service Implementation

## Goal

### Feature Goal
Create a comprehensive system for ride sharing services with production-ready schemas, custom admin components for driver management and ride analytics, and beautiful client pages for ride booking and tracking.

### Deliverable
A complete implementation including:
1. Production-ready Zod schema for ride sharing services with all necessary fields
2. Custom admin component for driver management and ride analytics that renders as a tab in the AutoAdmin panel
3. Beautiful, modern client page for ride booking and real-time tracking
...
- The rideSharing schema is properly defined with appropriate fields for drivers, vehicles, and ride tracking
- The admin component renders correctly in the AutoAdmin panel with functionality for managing drivers and analyzing rides
- The client page displays ride booking and tracking features in an intuitive and visually appealing way
...
existing_codebase_files:
  - file: src/lib/schema.ts
    ...
    relevance: This is where the rideSharing schema will be defined, following the existing pattern
    
  - file: src/components/ui/admin/hotel-management.tsx
    purpose: Example of a custom admin component for hotel schema
    relevance: This pattern will be replicated for the ride sharing admin component
  ...
  - file: src/components/pages/hotel/hotel-client-page.tsx
    ...
    relevance: This pattern will be replicated for the ride sharing client page with appropriate variations
  ...
  - file: src/routes/$businessName/index.tsx
    ...
    relevance: This file will need to be updated to handle the ride sharing business type
...
2. A businessType enum that includes "ride_sharing" 
3. A basic rideSharing schema defined in featureSchema but without a custom admin component
...
The problem identified is that we have a basic schema defined but no custom admin component or client page for ride sharing services.
...
```yaml
task_1_enhance_ride_sharing_schema:
  title: Enhance Ride Sharing Schema
  description: Add comprehensive fields to the existing ride sharing schema
  files:
    - src/lib/schema.ts
  details: |
    - Enhance the existing rideSharingSchema with additional fields:
      ...
      * locationDetails (object with service area coordinates, etc.)
...
```yaml
task_2_create_ride_sharing_admin_component:
  ...
  files:
    ...
    - src/lib/schema.ts
  details: |
    - Create RideSharingManagement component following the HotelManagement pattern
    - Include sections for:
      * Driver management with profile editing and verification
...
```yaml
task_3_create_ride_sharing_client_page:
  title: Create Ride Sharing Client Page
  description: Create a beautiful client page for ride sharing services
  files:
    - src/components/pages/ride-sharing/ride-sharing-client-page.tsx
    - src/routes/$businessName/index.tsx
  details: |
    - Create RideSharingClientPage component following the HotelClientPage pattern
    - Include sections for:
      * Hero section with ride booking form
      * Real-time ride booking with location services
      * Estimated pricing display
      * Driver tracking map with real-time updates
      * Ride history
...
task_4_update_autoadmin_integration:
  ...
  files:
    ...
    - src/lib/schema.ts
  details: |
    - Verify that the component loading mechanism works for the rideSharing schema
...
task_5_update_business_routing:
  ...
  description: Update the business route to handle the ride sharing business type
  files:
    - src/routes/$businessName/index.tsx
  details: |
    - Add case for "ride_sharing" in the switch statement
    - Ensure proper fallback to generic client page if needed
    - Test business type routing

task_6_implement_data_validation:
  title: Implement Data Validation
  description: Add proper validation and error handling for the ride sharing schema
  files:
    - src/lib/schema.ts
  details: |
    - Add custom validation where needed for ride sharing specific requirements
...
pnpm biome format --write src/lib/schema.ts src/components/ui/admin/ride-sharing-management.tsx src/components/pages/ride-sharing/ride-sharing-client-page.tsx

# Run Biome linter to catch potential issues
pnpm biome lint src/lib/schema.ts src/components/ui/admin/ride-sharing-management.tsx src/components/pages/ride-sharing/ride-sharing-client-page.tsx
```
...
# pnpm test src/components/ui/admin/ride-sharing-management.test.tsx
# pnpm test src/components/pages/ride-sharing/ride-sharing-client-page.test.tsx
```
...
# Manual checks:
# 1. The rideSharing schema is properly defined and exported
# 2. Admin component renders correctly in AutoAdmin panel
# 3. Client page renders correctly for ride sharing business type
...
6. Ensure driver and passenger location data is properly secured
...
9. Test real-time location tracking functionality
...
6. Monitor real-time location tracking performance
...
9. Optimize map rendering and location tracking performance