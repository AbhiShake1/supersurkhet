---
document_type: prp
title: Cinema/Theater Implementation
version: 1.0
author: AI IDE Agent
date: 2025-08-23
status: draft
...
# PRP: Cinema/Theater Implementation

## Goal

### Feature Goal
Create a comprehensive system for cinema and theater businesses with production-ready schemas, custom admin components for movie scheduling and ticket sales, and beautiful client pages for movie browsing and ticket booking.

### Deliverable
A complete implementation including:
1. Production-ready Zod schema for cinema businesses with all necessary fields
2. Custom admin component for movie scheduling and ticket sales that renders as a tab in the AutoAdmin panel
3. Beautiful, modern client page for movie browsing, showtime viewing, and ticket booking
...
- The cinema schema is properly defined with appropriate fields for movies, showtimes, and concession items
- The admin component renders correctly in the AutoAdmin panel with functionality for managing movies, showtimes, and ticket sales
- The client page displays movies and showtimes in an intuitive and visually appealing way with ticket booking capability
...
existing_codebase_files:
  - file: src/lib/schema.ts
    ...
    relevance: This is where the cinema schema will be defined, following the existing pattern
    
  - file: src/components/ui/admin/hotel-management.tsx
    purpose: Example of a custom admin component for hotel schema
    relevance: This pattern will be replicated for the cinema admin component
  ...
  - file: src/components/pages/hotel/hotel-client-page.tsx
    ...
    relevance: This pattern will be replicated for the cinema client page with appropriate variations
  ...
  - file: src/routes/$businessName/index.tsx
    ...
    relevance: This file will need to be updated to handle the cinema business type
...
2. A businessType enum that includes "cinema" 
3. A basic cinema schema defined in featureSchema but without a custom admin component
...
The problem identified is that we have a basic schema defined but no custom admin component or client page for cinema businesses.
...
```yaml
task_1_enhance_cinema_schema:
  title: Enhance Cinema Schema
  description: Add comprehensive fields to the existing cinema schema
  files:
    - src/lib/schema.ts
  details: |
    - Enhance the existing cinemaSchema with additional fields:
...
```yaml
task_2_create_cinema_admin_component:
  ...
  files:
    ...
    - src/lib/schema.ts
  details: |
    - Create CinemaManagement component following the HotelManagement pattern
    - Include sections for:
      * Movie scheduling with showtime management
      * Screen management
...
```yaml
task_3_create_cinema_client_page:
  ...
  details: |
    - Create CinemaClientPage component following the HotelClientPage pattern
    - Include sections for:
      * Hero section with featured movies and trailers
      * Now showing movies with filtering
      * Showtime schedule with seat availability
      * Ticket booking system with seat selection
      * Concession menu with ordering capability
      * Upcoming releases
...
task_4_update_autoadmin_integration:
  ...
  files:
    ...
    - src/lib/schema.ts
  details: |
    - Verify that the component loading mechanism works for the cinema schema
...
task_5_update_business_routing:
  ...
  description: Update the business route to handle the cinema business type
  files:
    - src/routes/$businessName/index.tsx
  details: |
    - Add case for "cinema" in the switch statement
    - Ensure proper fallback to generic client page if needed
    - Test business type routing

task_6_implement_data_validation:
  title: Implement Data Validation
  description: Add proper validation and error handling for the cinema schema
  files:
    - src/lib/schema.ts
  details: |
    - Add custom validation where needed for cinema specific requirements
...
pnpm biome format --write src/lib/schema.ts src/components/ui/admin/cinema-management.tsx src/components/pages/cinema/cinema-client-page.tsx

# Run Biome linter to catch potential issues
pnpm biome lint src/lib/schema.ts src/components/ui/admin/cinema-management.tsx src/components/pages/cinema/cinema-client-page.tsx
```
...
# pnpm test src/components/ui/admin/cinema-management.test.tsx
# pnpm test src/components/pages/cinema/cinema-client-page.test.tsx
```
...
# Manual checks:
# 1. The cinema schema is properly defined and exported
# 2. Admin component renders correctly in AutoAdmin panel
# 3. Client page renders correctly for cinema business type
