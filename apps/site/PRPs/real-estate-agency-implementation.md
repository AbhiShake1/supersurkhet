---
document_type: prp
title: Real Estate Agency Implementation
version: 1.0
author: AI IDE Agent
date: 2025-08-23
status: draft
...
# PRP: Real Estate Agency Implementation

## Goal

### Feature Goal
Create a comprehensive system for real estate agencies with production-ready schemas, custom admin components for property management and client inquiries, and beautiful client pages for property browsing and inquiries.

### Deliverable
A complete implementation including:
1. Production-ready Zod schema for real estate agencies with all necessary fields
2. Custom admin component for property management and client inquiries that renders as a tab in the AutoAdmin panel
3. Beautiful, modern client page for property browsing and inquiry submission
...
- The propertyListing schema is properly enhanced with appropriate fields for real estate agencies
- The admin component renders correctly in the AutoAdmin panel with functionality for managing properties and client inquiries
- The client page displays properties and real estate services in an intuitive and visually appealing way with inquiry submission capability
...
existing_codebase_files:
  - file: src/lib/schema.ts
    ...
    relevance: This is where the propertyListing schema will be enhanced, following the existing pattern
    
  - file: src/components/ui/admin/hotel-management.tsx
    purpose: Example of a custom admin component for hotel schema
    relevance: This pattern will be replicated for the real estate agency admin component
  ...
  - file: src/components/pages/hotel/hotel-client-page.tsx
    ...
    relevance: This pattern will be replicated for the real estate agency client page with appropriate variations
  ...
  - file: src/routes/$businessName/index.tsx
    ...
    relevance: This file will need to be updated to handle the real estate business type
...
2. A businessType enum that includes "real_estate" 
3. A propertyListingSchema that can be enhanced for real estate agencies
...
The problem identified is that we need to enhance the existing propertyListing schema and create custom admin/client components for real estate agencies.
...
```yaml
task_1_enhance_property_listing_schema:
  title: Enhance Property Listing Schema
  description: Add comprehensive fields to the existing property listing schema for real estate agencies
  files:
    - src/lib/schema.ts
  details: |
    - Enhance the existing propertyListingSchema with additional fields for real estate agencies:
      * listingType (enum) - Type of listing (sale, rent)
      * propertyType (enum) - Type of property (land, house, apartment, commercial)
      * size (string) - Size of property (e.g., '1200 sq. ft.' or '5 aana')
      ...
      * locationDetails (object with address, coordinates, neighborhood info)
      * propertyFeatures (record of feature names to boolean)
      * nearbyFacilities (record of facility names to distance)
      * propertyDocuments (record of document names to URLs)
...
```yaml
task_2_create_real_estate_admin_component:
  title: Create Real Estate Agency Admin Component
  description: Create a custom admin component for real estate agency management
  files:
    - src/components/ui/admin/real-estate-management.tsx
    - src/lib/schema.ts
  details: |
    - Create RealEstateManagement component following the HotelManagement pattern
    - Include sections for:
      ...
      * Market analytics
    - Register component in propertyListingSchema components function
...
```yaml
task_3_create_real_estate_client_page:
  title: Create Real Estate Agency Client Page
  description: Create a beautiful client page for real estate agencies
  files:
    - src/components/pages/real-estate/real-estate-client-page.tsx
    - src/routes/$businessName/index.tsx
  details: |
    - Create RealEstateClientPage component following the HotelClientPage pattern
    - Include sections for:
      * Hero section with property search and featured listings
      * Property catalog with advanced filtering and sorting
      * Property details with virtual tours/images
    ...
    - Update the business route to handle real estate business type
```
...
task_4_update_autoadmin_integration:
  ...
  files:
    ...
    - src/lib/schema.ts
  details: |
    - Verify that the component loading mechanism works for the propertyListing schema
...
task_5_update_business_routing:
  ...
  description: Update the business route to handle the real estate business type
  files:
    - src/routes/$businessName/index.tsx
  details: |
    - Add case for "real_estate" in the switch statement
    - Ensure proper fallback to generic client page if needed
    - Test business type routing

task_6_implement_data_validation:
  title: Implement Data Validation
  description: Add proper validation and error handling for the real estate agency schema
  files:
    - src/lib/schema.ts
  details: |
    - Add custom validation where needed for real estate agency specific requirements
...
task_7_performance_optimization:
  ...
  details: |
    ...
    - Optimize property catalog filtering and sorting
...
pnpm biome format --write src/lib/schema.ts src/components/ui/admin/real-estate-management.tsx src/components/pages/real-estate/real-estate-client-page.tsx

# Run Biome linter to catch potential issues
pnpm biome lint src/lib/schema.ts src/components/ui/admin/real-estate-management.tsx src/components/pages/real-estate/real-estate-client-page.tsx
```
...
# pnpm test src/components/ui/admin/real-estate-management.test.tsx
# pnpm test src/components/pages/real-estate/real-estate-client-page.test.tsx
```
...
# Manual checks:
# 1. The propertyListing schema is properly enhanced and exported
# 2. Admin component renders correctly in AutoAdmin panel
# 3. Client page renders correctly for real estate business type
...
# 1. Create a real estate agency business
# 2. Verify admin component renders in the panel
# 3. Test all functionality in admin component
# 4. Visit client page for real estate agency business
...
6. Ensure property and client data is properly secured
...
9. Test property search and filtering functionality
...
6. Monitor property inquiry conversion rates
...
9. Optimize property catalog filtering and sorting performance