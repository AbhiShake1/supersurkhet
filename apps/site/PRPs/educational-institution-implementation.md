...
# PRP: Educational Institution Implementation

## Goal

### Feature Goal
Create a comprehensive system for educational institutions with production-ready schemas, custom admin components for student management and course enrollment, and beautiful client pages for course browsing and enrollment.

### Deliverable
A complete implementation including:
1. Production-ready Zod schema for educational institutions with all necessary fields
2. Custom admin component for student management and course enrollment that renders as a tab in the AutoAdmin panel
3. Beautiful, modern client page for course browsing, enrollment, and educational resources
...
- The studentProfile schema is properly enhanced with appropriate fields for educational institutions
- The admin component renders correctly in the AutoAdmin panel with functionality for managing students and courses
- The client page displays courses and educational resources in an intuitive and visually appealing way with enrollment capability
...
existing_codebase_files:
  - file: src/lib/schema.ts
    ...
    relevance: This is where the studentProfile schema will be enhanced, following the existing pattern
    
  - file: src/components/ui/admin/hotel-management.tsx
    purpose: Example of a custom admin component for hotel schema
    relevance: This pattern will be replicated for the educational institution admin component
  ...
  - file: src/components/pages/hotel/hotel-client-page.tsx
    ...
    relevance: This pattern will be replicated for the educational institution client page with appropriate variations
  ...
  - file: src/routes/$businessName/index.tsx
    ...
    relevance: This file will need to be updated to handle the education business type
...
2. A businessType enum that includes "education" 
3. A studentProfileSchema that can be enhanced for educational institutions
...
The problem identified is that we need to enhance the existing studentProfile schema and create custom admin/client components for educational institutions.
...
```yaml
task_1_enhance_student_profile_schema:
  title: Enhance Student Profile Schema
  description: Add comprehensive fields to the existing student profile schema for educational institutions
  files:
    - src/lib/schema.ts
  details: |
    - Enhance the existing studentProfileSchema with additional fields for educational institutions:
      * classId (string) - ID of the class/student's current class
      * rollNumber (string) - Student's roll number
      * dateOfBirth (string) - Student's date of birth
      * guardianInfo (object with guardian details)
      * address (string) - Student's address
      * contactInfo (object with phone, email, etc.)
      * enrollmentDate (string) - Date of enrollment
      * academicRecords (record of subject names to grades)
      * attendance (record of dates to attendance status)
      * extracurricularActivities (record of activity names to boolean)
...
```yaml
task_2_create_educational_admin_component:
  title: Create Educational Institution Admin Component
  description: Create a custom admin component for educational institution management
  files:
    - src/components/ui/admin/educational-management.tsx
    - src/lib/schema.ts
  details: |
    - Create EducationalManagement component following the HotelManagement pattern
    - Include sections for:
      * Student management with profile editing
    ...
    - Register component in studentProfileSchema components function
...
```yaml
task_3_create_educational_client_page:
  title: Create Educational Institution Client Page
  description: Create a beautiful client page for educational institutions
  files:
    - src/components/pages/education/education-client-page.tsx
    - src/routes/$businessName/index.tsx
  details: |
    - Create EducationClientPage component following the HotelClientPage pattern
    - Include sections for:
      * Hero section with institution information and values
      * Course catalog with filtering and sorting
      * Enrollment process with form submission
    ...
    - Update the business route to handle education business type
```
...
task_4_update_autoadmin_integration:
  ...
  files:
    ...
    - src/lib/schema.ts
  details: |
    - Verify that the component loading mechanism works for the studentProfile schema
...
task_5_update_business_routing:
  ...
  description: Update the business route to handle the education business type
  files:
    - src/routes/$businessName/index.tsx
  details: |
    - Add case for "education" in the switch statement
    - Ensure proper fallback to generic client page if needed
    - Test business type routing

task_6_implement_data_validation:
  title: Implement Data Validation
  description: Add proper validation and error handling for the educational institution schema
  files:
    - src/lib/schema.ts
  details: |
    - Add custom validation where needed for educational institution specific requirements
...
task_7_performance_optimization:
  ...
  details: |
    ...
    - Optimize course catalog filtering and sorting
...
pnpm biome format --write src/lib/schema.ts src/components/ui/admin/educational-management.tsx src/components/pages/education/education-client-page.tsx

# Run Biome linter to catch potential issues
pnpm biome lint src/lib/schema.ts src/components/ui/admin/educational-management.tsx src/components/pages/education/education-client-page.tsx
```
...
# pnpm test src/components/ui/admin/educational-management.test.tsx
# pnpm test src/components/pages/education/education-client-page.test.tsx
```
...
# Manual checks:
# 1. The studentProfile schema is properly enhanced and exported
# 2. Admin component renders correctly in AutoAdmin panel
# 3. Client page renders correctly for education business type
...
# 1. Create an educational institution business
# 2. Verify admin component renders in the panel
# 3. Test all functionality in admin component
# 4. Visit client page for educational institution business
...
6. Ensure student data is properly secured and private
7. Implement proper data encryption for sensitive information
8. Ensure compliance with educational data privacy regulations
...
9. Test enrollment and academic record functionality
...
6. Monitor enrollment conversion rates
...
9. Optimize course catalog filtering and sorting performance