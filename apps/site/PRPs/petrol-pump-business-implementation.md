---
document_type: prp
title: Petrol Pump Business Implementation
version: 1.0
author: AI IDE Agent
date: 2025-08-23
status: draft
...
# PRP: Petrol Pump Business Implementation

## Goal

### Feature Goal
Create a comprehensive system for petrol pump businesses with production-ready schemas, custom admin components for fuel management and pricing, and beautiful client pages for displaying fuel prices and services.

### Deliverable
A complete implementation including:
1. Production-ready Zod schema for petrol pump businesses with all necessary fields
2. Custom admin component for fuel management and pricing that renders as a tab in the AutoAdmin panel
3. Beautiful, modern client page for displaying fuel prices and services
...
- The petrolPump schema is properly defined with appropriate fields for fuel types, pricing, and services
- The admin component renders correctly in the AutoAdmin panel with functionality for managing fuel prices and services
- The client page displays fuel prices in an intuitive and visually appealing way
...
existing_codebase_files:
  - file: src/lib/schema.ts
    ...
    relevance: This is where the petrolPump schema will be defined, following the existing pattern
    
  - file: src/components/ui/admin/hotel-management.tsx
    purpose: Example of a custom admin component for hotel schema
    relevance: This pattern will be replicated for the petrol pump admin component
  ...
  - file: src/components/pages/hotel/hotel-client-page.tsx
    ...
    relevance: This pattern will be replicated for the petrol pump client page with appropriate variations
  ...
  - file: src/routes/$businessName/index.tsx
    ...
    relevance: This file will need to be updated to handle the petrol pump business type
...
2. A businessType enum that includes "petrol_pump" 
3. A basic petrolPump schema defined in featureSchema but without a custom admin component
...
The problem identified is that we have a basic schema defined but no custom admin component or client page for petrol pump businesses.
...
```yaml
task_1_enhance_petrol_pump_schema:
  title: Enhance Petrol Pump Schema
  description: Add comprehensive fields to the existing petrolPump schema
  files:
    - src/lib/schema.ts
  details: |
    - Enhance the existing petrolPumpSchema with additional fields:
...
```yaml
task_2_create_petrol_pump_admin_component:
  ...
  files:
    ...
    - src/lib/schema.ts
  details: |
    - Create PetrolPumpManagement component following the HotelManagement pattern
    - Include sections for:
      * Fuel price management with real-time updating
      * Service management
...
```yaml
task_3_create_petrol_pump_client_page:
  ...
  details: |
    - Create PetrolPumpClientPage component following the HotelClientPage pattern
    - Include sections for:
...
task_4_update_autoadmin_integration:
  ...
  files:
    ...
    - src/lib/schema.ts
  details: |
    - Verify that the component loading mechanism works for the petrolPump schema
...
task_5_update_business_routing:
  ...
  description: Update the business route to handle the petrol pump business type
  files:
    - src/routes/$businessName/index.tsx
  details: |
    - Add case for "petrol_pump" in the switch statement
    - Ensure proper fallback to generic client page if needed
    - Test business type routing

task_6_implement_data_validation:
  title: Implement Data Validation
  description: Add proper validation and error handling for the petrol pump schema
  files:
    - src/lib/schema.ts
  details: |
    - Add custom validation where needed for petrol pump specific requirements
...
pnpm biome format --write src/lib/schema.ts src/components/ui/admin/petrol-pump-management.tsx src/components/pages/petrol-pump/petrol-pump-client-page.tsx

# Run Biome linter to catch potential issues
pnpm biome lint src/lib/schema.ts src/components/ui/admin/petrol-pump-management.tsx src/components/pages/petrol-pump/petrol-pump-client-page.tsx
```
...
# pnpm test src/components/ui/admin/petrol-pump-management.test.tsx
# pnpm test src/components/pages/petrol-pump/petrol-pump-client-page.test.tsx
```
...
# Manual checks:
# 1. The petrolPump schema is properly defined and exported
# 2. Admin component renders correctly in AutoAdmin panel
# 3. Client page renders correctly for petrol pump business type
