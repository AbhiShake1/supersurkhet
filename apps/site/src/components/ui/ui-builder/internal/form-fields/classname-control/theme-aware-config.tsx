...
function createRegularColorDropdown(
  ...
  return colorClasses.map((cls) => {
    ...
    return {
      value: cls,
      label: <DropdownOption color={colorClass}>{colorName}</DropdownOption>,
    };
  });
}
...
function createCombinedColorDropdown(
  ...
  const themeColors = createThemeAwareColorDropdown(
    shadcnClasses,
    classPrefix,
    bgPrefix,
  );
  const regularColors = createRegularColorDropdown(
    tailwindClasses,
    classPrefix,
    bgPrefix,
  );

  return [...themeColors, ...regularColors];
}
