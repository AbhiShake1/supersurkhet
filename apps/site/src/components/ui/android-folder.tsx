...
const AndroidFolder = ({ children }: { children: React.ReactNode }) => {
  ...
  return (
    <FolderContext.Provider value={{ isOpen, setIsOpen, position, setPosition, folderRef, overlayRef }}>
      {children}
...
// --- Trigger Component ---
const AndroidFolderTrigger = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ children, className, ...props }, ref) => {
    ...
    const setRefs = React.useCallback((node: HTMLDivElement) => {
      (folderRef as React.MutableRefObject<HTMLDivElement | null>).current = node;
      if (typeof ref === 'function') ref(node);
      else if (ref) ref.current = node;
    }, [folderRef, ref]);

    return (
      <div ref={setRefs} onClick={handleFolderClick} className={cn(className)} {...props}>
        {children}
      </div>
    );
  }
...
// --- Content Component ---
const AndroidFolderContent = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ children, className, ...props }, ref) => {
    ...
    return createPortal(
      <div
        ref={setRefs}
        className={cn("fixed z-50 bg-popover border border-border rounded-3xl shadow-2xl p-6 min-w-80", className)}
        style={{
          left: position.x,
          top: position.y,
