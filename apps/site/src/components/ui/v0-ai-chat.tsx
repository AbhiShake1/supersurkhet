...
export function VercelV0Chat({
  ...
  const [lastWizardPromptKey, setLastWizardPromptKey] = useState<string | null>(
    null,
  );
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: scroll when thread updates; messages is intentional
  useEffect(() => {
    scrollToBottom();
  ...
  const wizardStageKey = wizard?.stageKey ?? '';
  // biome-ignore lint/correctness/useExhaustiveDependencies: reset search when wizard stage key changes
  useEffect(() => {
    setWizardSearch('');
  }, [wizardStageKey]);
  ...
  const handleWizardForward = () => {
    ...
    const forwardEcho = wizard.forwardEchoLabel?.trim();
    if (forwardEcho) {
      setMessages((prev) => [
        ...prev,
        {
  ...
  const orderedWizardOptions = (() => {
    ...
    const recommended = filteredWizardOptions.filter(
      (option) => option.recommended,
    );
    const nonRecommended = filteredWizardOptions.filter(
      (option) => !option.recommended,
    );
                  ...
                  <div className="rounded-lg border border-white/10 bg-black/20">
                    ...
                    <div className="border-t border-white/10 p-2">
                      <input
                        ...
                        value={wizardSearch}
                        onChange={(event) =>
                          setWizardSearch(event.target.value)
                        }
