...
function _TransactionManagement({ slug }: TransactionManagementProps) {
	...
	const { data: transactions = [], isLoading, error } = api.transaction.useGet({
		keys: [slug],
	});
	...
	const TransactionCard = ({ transaction }: { transaction: Transaction & { _?: { soul: string } } }) => {
		const party = parties.find((p) => p._?.soul === transaction.partyId);
		const invoice = invoices.find((i) => i._?.soul === transaction.invoiceId);
			...
			<div className="rounded-md border bg-card p-4 shadow-xs flex flex-col gap-3">
				<div className="flex items-start justify-between gap-2">
					<div className="flex items-center gap-3">
						...
						<div>
							<h3 className="font-semibold text-sm capitalize">{transaction.type}</h3>
							<Badge
					...
					<div className="text-right">
						<div className="font-bold text-sm">Rs. {transaction.amount?.toFixed(2)}</div>
						...
						<div className="flex items-center justify-between">
							<div>
								...
								<p className="text-2xl font-bold text-purple-600">
									Rs. {transactions.reduce((sum, t) => sum + (t.amount || 0), 0).toFixed(2)}
								</p>
							</div>
				...
				<TabsContent value="cards" className="space-y-4">
					<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
						{filteredTransactions.map((transaction) => (
							<TransactionCard key={transaction._?.soul} transaction={transaction} />
						))}
					</div>
