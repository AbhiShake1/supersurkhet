...
function _InventoryLedgerManagement({ slug }: InventoryLedgerManagementProps) {
	...
	const { data: inventoryLedgers = [], isLoading, error } = api.inventoryLedger.useGet({
		keys: [slug],
	});
	...
	if (error) {
		return (
			<div className="flex flex-col items-center justify-center p-8 text-center">
				...
				<h3 className="text-lg font-medium text-gray-900 mb-1">Error loading inventory ledger</h3>
	...
	const InventoryLedgerCard = ({ ledger }: { ledger: InventoryLedger & { _?: { soul: string } } }) => {
		const product = products.find((p) => p._?.soul === ledger.productId);
		const invoice = invoices.find((i) => i._?.soul === ledger.invoiceId);
			...
			<div className="rounded-md border bg-card p-4 shadow-xs flex flex-col gap-3">
				<div className="flex items-start justify-between gap-2">
					<div className="flex items-center gap-3">
						...
						<div>
							<h3 className="font-semibold text-sm">{product?.name || "Product"}</h3>
							<Badge
					...
					<div className="text-right">
						<div className="font-bold text-sm">
							{type === "in" ? "+" : "-"}{type === "in" ? ledger.quantityIn : ledger.quantityOut}
						</div>
				...
				<div className="space-y-2 text-sm">
					...
					<div className="flex items-center gap-2">
						<span className="text-gray-500">Quantity {type === "in" ? "In" : "Out"}:</span>
						<span>{type === "in" ? ledger.quantityIn : ledger.quantityOut}</span>
					</div>
					<div className="flex items-center gap-2">
						...
						<div className="flex items-center justify-between">
							<div>
								...
								<p className="text-2xl font-bold text-purple-600">
									{inventoryLedgers.reduce((sum, l) => sum + (l.quantityIn || 0) - (l.quantityOut || 0), 0)}
								</p>
							</div>
								...
								<div>
									...
									<div className="space-y-2">
										<div className="flex justify-between">
											<span>Total In</span>
											<span className="font-medium">
												{inventoryLedgers.reduce((sum, l) => sum + (l.quantityIn || 0), 0)}
											</span>
										</div>
										<div className="flex justify-between">
											<span>Total Out</span>
											<span className="font-medium">
												{inventoryLedgers.reduce((sum, l) => sum + (l.quantityOut || 0), 0)}
											</span>
										</div>
