...
const ReturnedProductsSchema = z.object({
  returnedProducts: z.array(
    z.object({
      ...
      quantity: z.number({ coerce: true }).int().nonnegative().describe("Quantity Returned"),
    })
...
export function TripManagement({ slug }: { slug: string }) {
  ...
  const [returnFormData, setReturnFormData] = useState<ReturnedProductsFormData>({ returnedProducts: [] });
  ...
  const handleSubmitReturn = (data: ReturnedProductsFormData) => {
    ...
    const soldProducts = currentTrip.products.map((dispatchedProduct: any) => {
      const returnedProduct = data.returnedProducts.find(rp => rp.productId === dispatchedProduct.productId);
                  ...
                  <CardHeader className="flex flex-row items-center justify-between">
                    <div>
                      <h3 className="font-semibold">
                        {vehicle?.name || "Unknown Vehicle"} - {vehicle?.licensePlate || "No Plate"}
                      </h3>
                      <p className="text-sm text-muted-foreground">
                        Dispatched: {format(new Date(trip.dispatchTime), 'MMM dd, yyyy HH:mm')}
                        {trip.returnTime && ` | Returned: ${format(new Date(trip.returnTime), 'MMM dd, yyyy HH:mm')}`}
                      </p>
                    </div>
                    <div className="flex items-center gap-2">
                      <Badge variant={trip.returnTime ? "default" : "secondary"}>
                        {status}
                      </Badge>
                      {!trip.returnTime && (
                        <Dialog open={returnDialogOpen && currentTrip?._?.soul === trip._?.soul} onOpenChange={setReturnDialogOpen}>
                          ...
                          <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
                            <DialogHeader>
                              <DialogTitle>Mark Return for {vehicle?.name || "Vehicle"}</DialogTitle>
                            </DialogHeader>
                            ...
                            <div className="space-y-4">
                                  ...
                                  return (
                                    <div key={idx} className="grid grid-cols-3 gap-2 text-sm">
                                      ...
                                      <div className="text-center">
                                        {returnFormData.returnedProducts[idx]?.quantity || 0}
                                      </div>
                                    </div>
                                  );
                                })}
                              ...
                              <div>
                                <h4 className="font-medium mb-2">Enter Returned Products:</h4>
                                <AutoForm
                                  ...
                                  fieldConfig={{
                                    returnedProducts: {
                                      ...
                                      fields: {
                                        productId: {
                                          fieldType: "select",
                                          options: products.map(p => [p._?.soul!, p.title]),
                                        },
                                        quantity: {
                                          fieldType: "number",
                                        },
