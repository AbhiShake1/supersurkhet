...
function getItemTitle(item: any): string {
  ...
  return item.title || item.name || item.label || item.text || item.displayName || item.heading || ""
}
...
export function BadgeMarquee<T>({ items, variant = "ghost", onSelected }: BadgeMarqueeProps<T>) {
  ...
  return (
    <>
      <div className="flex items-center gap-8 w-full overflow-hidden">
        ...
        {/* Marquee Container */}
        <div ref={containerRef} className="flex-3 relative overflow-x-auto scrollbar-hide no-scrollbar" onScroll={handleScroll}>
          <div className={`flex gap-3 ${!isUserScrolling ? "animate-marquee" : ""}`}>
            {displayItems?.map((item: any, index: number) => (
              ...
              >
                <img src={getItemImageUrl(item)} className="h-6 w-6 rounded-full" />
                {getItemTitle(item)}
              </Button>
            ))}
          </div>
      ...
      <Dialog open={!!selectedItem} onOpenChange={() => setSelectedItem(null)}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>{selectedItem ? getItemTitle(selectedItem) : ''}</DialogTitle>
            <DialogDescription className="mt-2">
              <div className="flex flex-col items-center">
                ...
                {selectedItem && getItemDescription(selectedItem) && (
                  <p className="text-gray-700 mb-2">{getItemDescription(selectedItem)}</p>
                )}
                {selectedItem && getItemPrice(selectedItem) && (
                  <p className="text-lg font-bold text-primary">Rs. {getItemPrice(selectedItem)}</p>
                )}
              </div>
            </DialogDescription>
          </DialogHeader>
