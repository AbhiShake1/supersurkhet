...
describe('autoform HMR contracts', () => {
  it('keeps AutoForm free of compiler memo directive', () => {
    ...
    expect(content).not.toMatch(/export function AutoForm[\s\S]*?'use memo';/);
  });

  it('keeps SelectField controlled by external form value', () => {
    const content = readSource(
      'components/ui/autoform/components/SelectField.tsx',
    );
    ...
    expect(content).toContain('onValueChange={handleValueChange}');
  });
});
