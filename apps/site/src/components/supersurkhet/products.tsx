...
const ProductImage = React.forwardRef<HTMLImageElement, ProductImageProps>(
  ({ children, className, isLoading, src, alt, ...props }, ref) => {
    ...
    if (isLoading) {
      return <Skeleton className={cn("h-40 w-full rounded-md", className)} {...props} />;
    }
    ...
    return (
      <img
        ref={ref}
        className={cn("w-full object-cover rounded-md aspect-square", className)}
        src={finalSrc}
...
const ProductTitle = React.forwardRef<HTMLHeadingElement, ProductTitleProps>(
  ({ className, isLoading, children, ...props }, ref) => {
    ...
    if (isLoading) {
      return <Skeleton className={cn("h-5 w-3/4 mb-2", className)} {...props} />;
    }

    return (
      <h3 ref={ref} className={cn("font-semibold text-lg mb-2", className)} {...props}>
        {product?.title}
      </h3>
    );
  }
...
interface ProductDescriptionProps extends React.HTMLAttributes<HTMLParagraphElement> {
  isLoading?: boolean;
}

const ProductDescription = React.forwardRef<HTMLParagraphElement, ProductDescriptionProps>(
  ({ className, isLoading, children, ...props }, ref) => {
    const { product } = useProduct();
    ...
    return (
      <p ref={ref} className={cn("text-sm text-muted-foreground mb-3 flex-grow", className)} {...props}>
        {product?.description}
      </p>
    );
  }
...
const ProductPrice = React.forwardRef<HTMLDivElement, ProductPriceProps>(
  ({ className, isLoading, ...props }, ref) => {
    ...
    if (isLoading) {
      return <Skeleton className={cn("h-5 w-1/3 mb-3", className)} {...props} />;
    }
...
const ProductActions = React.forwardRef<HTMLDivElement, ProductActionsProps>(
  ({ className, isLoading, children, ...props }, ref) => {
    if (isLoading) {
      return <Skeleton className={cn("h-10 w-full rounded", className)} {...props} />;
    }
