...
export interface CarouselItemProps extends React.HTMLAttributes<HTMLAnchorElement> {
...
export const CarouselItemComponent = React.forwardRef<HTMLAnchorElement, CarouselItemProps>(
  ({ item = defaultItem, variant = "default", children, onItemClick, ...props }, ref) => {
    ...
    const { id, imageSrc, href, imageAlt, title, description, tag, discountPercentage, brandLogoSrc, brandName, promoCode, rating } = item || defaultItem;

    return (
      <TooltipProvider>
        <motion.div whileHover={{ y: -8 }} transition={{ type: "spring", stiffness: 300, damping: 20 }} className="relative" key={id}>
          <Card className={cn("overflow-hidden group transition-all duration-300 hover:shadow-xl border-border", cardVariants[variant])}>
            <a ref={ref} href={href} className="block h-full" onClick={handleClick} {...props}>
              <div className="relative h-1/2 overflow-hidden">
                <AspectRatio ratio={16 / 9}>
                  <img src={imageSrc} alt={imageAlt || title || "Carousel item"} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110" onError={(e) => { e.currentTarget.src = defaultItem.imageSrc }} />
                </AspectRatio>
                {discountPercentage && (
                  <Badge className="absolute top-3 left-3 bg-red-500 hover:bg-red-600 text-white border-none">-{discountPercentage}%</Badge>
                )}
                {tag && (
                ...
                {children || (
                  <>
                    <div className="space-y-3">
                      {title && <h3 className="text-xl font-bold text-foreground leading-tight line-clamp-2">{title}</h3>}
                      {description && <p className="text-sm text-muted-foreground line-clamp-2">{description}</p>}
                      {rating && (
                        <div className="flex items-center gap-2">
                          <div className="flex">
                            {[...Array(5)].map((_, i) => (
                              <span key={i} className={cn("text-lg", i < Math.floor(rating) ? "text-yellow-500" : "text-muted")}>★</span>
                            ))}
                          </div>
                    ...
                    {(brandName || brandLogoSrc || promoCode) && (
                      <div className="flex items-center justify-between pt-4 border-t">
                          ...
                          <div className="flex items-center gap-3">
                            {brandLogoSrc && (
                              <div className="relative w-10 h-10 rounded-full overflow-hidden border">
                                <img src={brandLogoSrc} alt={`${brandName || 'Brand'} logo`} className="w-full h-full object-cover" onError={(e) => { e.currentTarget.style.display = 'none'; }} />
                              </div>
                            )}
                            <div>
                              {brandName && <p className="text-sm font-semibold text-foreground">{brandName}</p>}
                              {promoCode && (
                                <Tooltip>
                                  <TooltipTrigger asChild>
                                    <p className="text-xs text-muted-foreground cursor-help">Code: {promoCode}</p>
                                  </TooltipTrigger>
                                  <TooltipContent>
                        ...
                        <Button size="icon" className="rounded-full transition-all duration-300 group-hover:rotate-[-45deg] group-hover:bg-primary" variant="outline">
...
//CarouselCard
export interface CarouselCardProps extends React.HTMLAttributes<HTMLAnchorElement> {
  item?: CarouselItemType;
  variant?: "default" | "compact" | "expanded";
  children?: React.ReactNode;
  onItemClick?: (item: CarouselItemType) => void;
...
export const CarouselCard = React.forwardRef<HTMLAnchorElement, CarouselCardProps>(
  ({ item, variant = "default", children, onItemClick, ...props }, ref) => {
    return <CarouselItemComponent item={item} variant={variant} children={children} onItemClick={onItemClick} ref={ref} {...props} />;
  }
);
...
export const Carousel = React.forwardRef<HTMLDivElement, CarouselProps>(
  ({ items, children, defaultChildren = true, className, showProgress = false, autoPlay = false, variant = "default", title = "Featured Items", subtitle = "Discover amazing content", showNavigation = true, onItemClick, ...props }, ref) => {
    ...
    React.useEffect(() => {
      ...
      const interval = setInterval(() => {
        if (scrollContainerRef.current) {
          ...
          if (scrollLeft >= scrollWidth - clientWidth) container.scrollTo({ left: 0, behavior: "smooth" });
        ...
        {(title || subtitle) && (
          <div className="flex justify-between items-center mb-4">
            ...
            {showNavigation && getItemsToRender.length > 1 && (
              <div className="flex items-center gap-2">
                <Button variant="outline" size="icon" onClick={() => scroll("left")} className="rounded-full" disabled={currentIndex === 0}><ChevronLeft className="h-4 w-4" /></Button>
                <Button variant="outline" size="icon" onClick={() => scroll("right")} className="rounded-full" disabled={currentIndex === getItemsToRender.length - 1}><ChevronRight className="h-4 w-4" /></Button>
              </div>
            )}
          </div>
        )}
        <div ref={scrollContainerRef} className="flex gap-4 overflow-x-auto pb-6 scrollbar-hide snap-x snap-mandatory scroll-smooth" onScroll={updateScrollProgress} style={{ scrollBehavior: "smooth" }}>
          {children || getItemsToRender.map((item) => <CarouselItemComponent key={item.id} item={item} variant={variant} onItemClick={onItemClick} />)}
        </div>
          ...
          <div className="mt-4 space-y-2">
            ...
            <div className="flex justify-center gap-1">
              {getItemsToRender.map((_, index) => (
                <button key={index} onClick={() => scrollToIndex(index)} className={cn("w-2 h-2 rounded-full transition-all", index === currentIndex ? "bg-primary w-4" : "bg-muted hover:bg-muted-foreground/50")} aria-label={`Go to item ${index + 1}`} />
              ))}
            </div>
          </div>
        )}
...
export const EnhancedCarousel = React.forwardRef<HTMLDivElement, EnhancedCarouselProps>(
        ...
        {showFilters && filterOptions && (
          <div className="flex gap-3 mb-4">
            {["all", ...(filterOptions.tags || [])].map((tag) => (
              <Button key={tag} size="sm" variant={activeFilter === tag ? "default" : "outline"} onClick={() => handleFilterClick(tag)}>
                {tag}
              </Button>
            ))}
          </div>
