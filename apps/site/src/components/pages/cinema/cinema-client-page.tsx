...
interface CartContextType {
  items: CartItem[];
  addItem: (item: Snack, quantity: number) => void;
...
export function CartProvider({ children }: { children: React.ReactNode }) {
  ...
  const addItem = (item: Snack, quantity: number) => {
    setItems((prevItems) => {
...
interface SnackCardProps {
  snack: Snack;
  index: number;
  isSnacksInView: boolean;
}

function SnackCard({ snack, index, isSnacksInView }: SnackCardProps) {
    ...
    >
      <Card className="text-center h-full border border-border rounded-2xl overflow-hidden hover:shadow-lg transition-all duration-300">
        ...
        <CardFooter className="pb-6 flex flex-col gap-3">
          ...
          <Button
            size="sm"
            className="w-full py-5 text-base bg-gradient-to-r from-primary to-primary/80 hover:from-primary/90 hover:to-primary/70 text-primary-foreground transition-all duration-300"
            onClick={addToCart}
...
function CinemaClientPagePresenter({ slug }: CinemaClientPageProps) {
            ...
            >
              {[...Array(5)].map((_, i) => (
                <Star
                  ...
                  className={`w-6 h-6 ${i < Math.floor(cinemaInfo.rating)
              ...
              <section ref={snacksRef}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {snacks.map((snack, index) => (
                    <SnackCard snack={snack} key={snack.id} index={index} isSnacksInView={isSnacksInView} />
                  ))}
                </div>
              </section>
                ...
                <CredenzaHeader>
                  <CredenzaTitle asChild>
                    <div className="flex items-center justify-between mb-4">
                      <h2 className="text-2xl font-bold">Your Snacks</h2>
                      {items.length > 0 && (
                        <Button
                          variant="ghost"
                          size="sm"
