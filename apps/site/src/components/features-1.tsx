...
export const FeaturesSchema = z.object({
  ...
  subtitle: z.string().default("Libero sapiente aliquam quibusdam aspernatur, praesentium iusto repellendus."),
...
export default function Features({
  ...
  return (
    <section className={`bg-zinc-50 py-16 md:py-32 dark:bg-transparent ${className}`}>
          ...
          {features.map((feature, index) => (
            <Card key={index} className="group shadow-zinc-950/5">
              <CardHeader className="pb-3">
                <CardDecorator>
                  {React.createElement(iconMap[feature.icon as keyof typeof iconMap] || Zap, {
                    className: "size-6",
                    'aria-hidden': true
                  })}
                </CardDecorator>
