...
interface AppSidebarProps {
  ...
  setSelectedElement: (element: LayoutElement | null) => void;
...
export function AppSidebar({
  ...
  const [floorColor, setFloorColor] = useState<string>(
    currentFloor.backgroundColor || '#ffffff',
  );
  ...
  if (previousSelectedElementIdRef.current !== (selectedElement?.id ?? null)) {
    ...
    if (selectedElement) {
      ...
      setCapacity(selectedElement.capacity || 1);
  ...
  const handleAddOrUpdateElement = () => {
    if (selectedElement && onUpdateElement) {
      ...
      onUpdateElement(selectedElement.id, {
        ...
        capacity: ['chair', 'sofa', 'table'].includes(elementType)
          ? capacity
          : 0,
        label,
    ...
    // Reset label if adding new element
    if (!selectedElement) {
      setLabel('');
    }
  };
              ...
              <AccordionItem value="areas">
                ...
                <AccordionContent>
                  <div className="grid grid-cols-3 gap-2">
                    ...
                    <Button
                      variant={
                        elementType === 'kitchen' ? 'default' : 'outline'
                      }
                      className={cn(
                        'flex flex-col items-center justify-center h-20 gap-1',
                      )}
                    ...
                    <Button
                      variant={
                        elementType === 'bathroom' ? 'default' : 'outline'
                      }
                      className={cn(
                        'flex flex-col items-center justify-center h-20 gap-1',
                      )}
            ...
            <div className="space-y-4 p-2">
              <div className="space-y-2">
                ...
                <Select
                  value={elementSize}
                  onValueChange={(value) =>
                    setElementSize(value as ElementSize)
                  }
                >
                  <SelectTrigger>
              ...
              <div className="space-y-2">
                <Button className="w-full" onClick={handleAddOrUpdateElement}>
                  {selectedElement ? 'Update Element' : 'Add to Layout'}
                </Button>
                <Button
                ...
                <div className="grid grid-cols-3 gap-2 text-center">
                    ...
                    <div className="text-lg font-semibold">
                      {
                        currentFloor.elements.filter((s) =>
                          ['chair', 'sofa', 'table'].includes(s.type),
                        ).length
                      }
                    </div>
                  ...
                  <div className="p-2 bg-muted rounded-md">
                    <div className="text-lg font-semibold">
                      {
                        currentFloor.elements.filter((s) =>
                          ['wall', 'door', 'window', 'stairs'].includes(s.type),
                        ).length
                      }
                    </div>
              ...
              <div className="space-y-2">
                ...
                <div className="p-3 bg-muted rounded-md text-center">
                  <div className="text-2xl font-bold">
                    {currentFloor.elements.reduce(
                      (total, element) => total + (element.capacity || 0),
                      0,
                    )}
                  </div>
