...
export function PreviewMode({ floor, darkMode }: PreviewModeProps) {
  ...
  const getElementShadow = (element: LayoutElement) => {
    ...
    const shadowSize =
      Math.max(element?.width ?? 0, element?.height ?? 0) * 0.1;
    ...
    <div className="relative w-full h-full overflow-hidden">
      ...
      <div className="absolute top-4 right-4 z-10 flex flex-col gap-2 bg-white dark:bg-gray-800 rounded-md shadow-md p-1">
        <Button
          ...
          className={
            viewMode === '2d' ? 'bg-primary text-primary-foreground' : ''
          }
          title="2D View"
        >
        ...
        <Button
          ...
          className={
            viewMode === '3d' ? 'bg-primary text-primary-foreground' : ''
          }
          title="3D View"
        >
      ...
      >
        <div
          ...
          style={{
            ...
            transform:
              viewMode === '3d'
                ? `rotateX(30deg) rotateZ(${rotation}deg) scale(0.8)`
                : `rotateZ(${rotation}deg)`,
            transformOrigin: 'center center',
            transformStyle: 'preserve-3d',
            boxShadow:
              viewMode === '3d' ? '0 20px 50px rgba(0, 0, 0, 0.3)' : 'none',
          }}
        >
          {/* Floor grid */}
          <div
