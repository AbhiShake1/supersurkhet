  ...
  const updateElements = (floorId: string, newElements: LayoutElement[]) => {
    updateLayoutWithHistory({
      ...
      floors: layout.floors.map((floor) =>
        floor.id === floorId ? { ...floor, elements: newElements } : floor,
      ),
    });
  };

  const updateFloorProperties = (
    floorId: string,
    properties: Partial<Floor>,
  ) => {
    updateLayoutWithHistory({
      ...layout,
      floors: layout.floors.map((floor) =>
        floor.id === floorId ? { ...floor, ...properties } : floor,
      ),
    });
  };

  const currentFloor =
    layout.floors.find((floor) => floor.id === activeFloor) || layout.floors[0];
  ...
  return (
    <SidebarProvider>
      ...
      >
        {/* Header */}
        <header
          className={`border-b p-4 ${darkMode ? 'bg-gray-800 border-gray-700' : 'bg-card'}`}
        >
          ...
          <div className="flex-1 flex flex-col overflow-hidden">
            <div className="flex">
              ...
              <AppSidebar
                ...
                selectedElement={currentFloor.elements.find(
              ...
              <div className="flex-1 flex flex-col overflow-hidden">
                {/* Floor Tabs */}
                <div
                  className={`border-b p-2 ${darkMode ? 'bg-gray-800/40 border-gray-700' : 'bg-muted/40'}`}
                >
                  <Tabs
                    value={activeFloor}
                  ...
                  >
                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                      <TabsList
                        className={`h-auto flex-wrap ${darkMode ? 'bg-gray-700' : ''}`}
                      >
                        ...
                        {previewMode ? (
                          ...
                          <LayoutCanvas
                            ...
                            onUpdateElements={(newElements) =>
                              updateElements(floor.id, newElements)
                            }
                            darkMode={darkMode}
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      if ((e.ctrlKey || e.metaKey) && e.key === 'z' && !e.shiftKey && canUndo) {
        ...
        ((e.ctrlKey || e.metaKey) && e.key === 'y' && canRedo) ||
