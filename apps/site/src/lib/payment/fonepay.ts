...
export async function requestQrCode(
	...
	if (!response.ok) {
		...
		throw new Error(
			`QR Request failed: ${errorBody.message || response.statusText}`,
		);
	}
...
export async function checkQrStatus(
	...
	if (!response.ok) {
		const errorBody = await response.json();
		throw new Error(
			`Check QR Status failed: ${errorBody.message || response.statusText}`,
		);
	}
...
export async function postTaxRefund(
	...
	if (!response.ok) {
		const errorBody = await response.json();
		throw new Error(
			`Post Tax Refund failed: ${errorBody.message || response.statusText}`,
		);
	}
