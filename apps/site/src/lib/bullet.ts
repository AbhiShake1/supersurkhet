...
export type BulletChain<S extends GunSchema, TExt extends object> =
  & IGunChain<S, any>
  & TExt
...
export type BulletPropsDeep<TNode extends Record<string, GunSchema>, TExt extends object> = {
...
export interface BulletExtensible<
  ...
  TExt extends object
> {
  extend<E extends object, ThisRef extends BulletRoot<TNode, TGunInstance, TExt>>(
    this: ThisRef,
...
> =
  ...
  & TExt
  & BulletExtensible<TNode, TGunInstance, TExt>
  & { keys: string[] }

export default function createBullet<
  const TNode extends Record<string, GunSchema>,
  const TGunInstance extends IGunInstanceRoot<TNode, any> = IGunInstanceRoot<TNode, any>,
>(gun: TGunInstance) {
        ...
        if (prop === "extend") {
          return function <E extends object>(factory: (thisRef: any) => E) {
            if (typeof factory !== "function") {
              throw new TypeError("extend(...) expects a function: (thisRef) => extensionObject");
            }
