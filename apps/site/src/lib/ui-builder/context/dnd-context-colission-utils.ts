...
export const createTransformAwareCollisionDetection =
  (): CollisionDetection => {
    return ({
      active,
      droppableRects,
        ...
        droppableContainers.forEach((container) => {
          ...
          if (element) {
            try {
              ...
              console.warn(
                'Failed to get fresh rectangle for:',
                container.id,
                error,
              );
            }
      ...
      const transformAdjustedX =
        (iframeRelativeX - transformState.positionX) / transformState.scale;
      const transformAdjustedY =
      ...
      if (pointerCollisions.length > 0) {
        if (Math.abs(iframeScrollOffset.y) > 50 || isAtTop) {
          console.debug(
            `Found pointer collisions with fresh rects ${isAtTop ? '(at top)' : '(mid-scroll)'}:`,
            pointerCollisions.map((c) => c.id),
          );
        }
        ...
        debugCollisionDetection(
          ...
          pointerCollisions,
          {
            method: 'pointerWithin',
            scrollState: isAtTop ? 'at top' : 'mid-scroll',
            ...debugInfo,
          },
        );
      ...
      if (rectCollisions.length > 0) {
        if (Math.abs(iframeScrollOffset.y) > 50 || isAtTop) {
          console.debug(
            `Found rect collisions with fresh rects ${isAtTop ? '(at top)' : '(mid-scroll)'}:`,
            rectCollisions.map((c) => c.id),
          );
        }
      ...
      if (simpleCollisions.length > 0) {
        if (Math.abs(iframeScrollOffset.y) > 50 || isAtTop) {
          console.debug(
            `Found collisions with simple coords ${isAtTop ? '(at top)' : '(mid-scroll)'}:`,
            simpleCollisions.map((c) => c.id),
          );
        }
      ...
      if (cachedCollisions.length > 0) {
        if (Math.abs(iframeScrollOffset.y) > 50 || isAtTop) {
          console.debug(
            `Found collisions with cached rects ${isAtTop ? '(at top)' : '(mid-scroll)'}:`,
            cachedCollisions.map((c) => c.id),
          );
        }
      ...
      if (
        (Math.abs(iframeScrollOffset.y) > 50 || isAtTop) &&
        originalCollisions.length === 0
      ) {
        console.debug(
          `No collisions found with any method ${isAtTop ? '(at top)' : '(mid-scroll)'}`,
        );
      }
