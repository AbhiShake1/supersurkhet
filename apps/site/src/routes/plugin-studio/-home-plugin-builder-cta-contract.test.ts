...
describe('home plugin builder call-to-action contract', () => {
  it('links from home to plugin studio without bootstrapped query params', () => {
    const content = getRouteContent();

    expect(content).toContain('to="/plugin-studio"');
    expect(content).toContain('Open Plugin Builder');
    expect(content).not.toContain('pluginBuilderEntrySearch');
    expect(content).not.toContain("tab: 'overview'");
    expect(content).not.toContain("pluginId: 'plugin.restaurant.admin'");
