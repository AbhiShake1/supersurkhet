...
function getPaymentStatus(totalPaid: number, totalCost: number) {
	if (totalPaid === totalCost) return "paid";
	if (totalPaid === 0) return "pending";
	if (totalPaid > totalCost) return "overpaid (invalid)";
	return `partial (${formatCurrency(totalCost - totalPaid)} to pay)`;
}
...
function refreshPaidAmount(form: UseFormReturn) {
	...
	form.setValue("paidAmounts", [
		{
			paidAmount: totalCost,
			paidOn: new Date().toISOString(),
		},
	]);
	form.setValue("paymentStatus", getPaymentStatus(totalCost, totalCost));
}
...
export function useStockImportsConfig({
	...
	const productsBySoul = useMemo(
		() =>
			new Map(products.filter((p) => p?._?.soul).map((p) => [p._!.soul!, p])),
		[products],
	);

	const partiesBySoul = useMemo(
		() =>
			new Map(parties.filter((p) => p?._?.soul).map((p) => [p._!.soul!, p])),
		[parties],
	);
	...
	return {
		...
		slug,
		group: "Inventory",
		formSchemaTransformer: (schema) =>
			schema.superRefine((stockImport, ctx) => {
				const totalPaid =
					stockImport.paidAmounts?.reduce(
				...
				if (totalPaid > totalCost)
					ctx.addIssue({
						code: "custom",
						message: `Total paid amount cannot be greater than total cost (${totalCost})`,
						path: ["paidAmounts"],
					});
			}),
		previewOverrides: {
		...
		onCreate(_, variables) {
			...
			const itemsByProductIdWithQuantity = variables.items?.reduce(
				(a, { product, quantity, unit }) => {
					...
					if (productInfo.unit && productInfo.unit.includes(":")) {
						...
						if (unit === unitType) {
							adjustedQuantity = quantity * Number.parseInt(piecesPerUnit, 10);
						}
					}
				...
				variables.items?.map((item, index) => {
					...
					if (productInfo?.unit && productInfo.unit.includes(":")) {
						...
						if (item.unit === unitType) {
							adjustedQuantity =
								item.quantity * Number.parseInt(piecesPerUnit, 10);
						}
					}
			...
			createInvoice({
				...
				tax: 0,
				paidAmounts: variables.paidAmount
					? [
							{
...
export function usePartyConfig({
	...
	const { mutate: deleteStockImport } = api.stockImport.useDelete({
		keys: [slug],
	});
...
export function useSalesConfig({
	...
	const customersBySoul = useMemo(
		() =>
			new Map(customers.filter((p) => p?._?.soul).map((p) => [p._!.soul!, p])),
		[customers],
	);
	...
	return {
		...
		formSchemaTransformer: (schema) =>
			schema.superRefine((sale, ctx) => {
				const totalPaid =
					sale.paidAmounts?.reduce(
						(sum, payment) => sum + payment.paidAmount,
						0,
					) || 0;
				if (totalPaid === 0) return;
				...
				.superRefine((items, ctx) => {
					items.forEach((item, index) => {
						...
						if (product.unit && product.unit.includes(":")) {
							...
							if (item.unit === unitType) {
								availableStock =
									product.stockQuantity * Number.parseInt(piecesPerUnit, 10);
							}
						}
											...
											if (product && product._?.soul) {
												...
												if (product.unit && product.unit.includes(":")) {
													...
													if (item.unit === unitType) {
														adjustedQuantity =
															item.quantity *
															Number.parseInt(piecesPerUnit, 10);
													}
												}
												updateProduct({
													id: product._.soul,
...
export function useInvoicesConfig({
	...
	const vehiclesBySoul = useMemo(
		() =>
			new Map(vehicles.filter((v) => v?._?.soul).map((v) => [v._!.soul!, v])),
		[vehicles],
	);
	...
	return {
		...
		actions: ({ row }) => {
			...
			const party =
				partiesBySoul.get(partyId) ||
				customersBySoul.get(partyId) ||
				vehiclesBySoul.get(partyId);
