  ...
  return (
    <Card className="border-border/70 bg-card/90">
      <CardHeader>
        ...
        <CardDescription>
          Modular V3 surfaces for trigger rules, action packages, diagnostics, and execution logs.
        </CardDescription>
      </CardHeader>
      <CardContent>
        <Tabs defaultValue="trigger-rules">
          <TabsList>
            ...
            <TabsTrigger value="action-package">Action Package Manager</TabsTrigger>
            <TabsTrigger value="diagnostics">Diagnostics</TabsTrigger>
            <TabsTrigger value="execution-logs">Execution Logs/Replay</TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="trigger-rules" className="space-y-3">
            {workflows.length === 0 ? (
              ...
              workflows.map((workflow) => (
                <div key={workflow.workflowId} className="rounded-md border p-3">
                  <p className="text-sm font-medium">{workflow.workflowId}</p>
                  <p className="text-xs text-muted-foreground">
                    {workflow.trigger?.table ?? workflow.table} • {workflow.trigger?.event ?? workflow.hook}
                  </p>
                </div>
              ))
            )}
          ...
          <TabsContent value="action-package" className="space-y-3">
            {actionManifest.length === 0 ? (
              ...
              actionManifest.map((action) => (
                <div key={action.actionId} className="flex items-center justify-between rounded-md border p-3">
                  <div>
                    <p className="text-sm font-medium">{action.actionId}</p>
                    <p className="text-xs text-muted-foreground">
                      runtime: {action.runtime ?? 'missing'} • capabilities: {(action.capabilities ?? []).length}
                    </p>
                  </div>
                  <Badge variant={action.runtime && (action.capabilities?.length ?? 0) > 0 ? 'default' : 'destructive'}>
                    {action.runtime && (action.capabilities?.length ?? 0) > 0 ? 'Ready' : 'Incomplete'}
                  </Badge>
                </div>
              ))
            )}
          ...
          <TabsContent value="diagnostics" className="space-y-3">
            ...
            {diagnostics.length === 0 ? (
              <p className="text-sm text-muted-foreground">No V3 gate diagnostics.</p>
            ) : (
              <div className="space-y-2">
                {diagnostics.map((diagnostic) => (
                  <div key={`${diagnostic.code}:${diagnostic.path.join('.')}`} className="rounded-md border p-3">
                    <p className="text-sm font-medium">{diagnostic.code}</p>
                    <p className="text-xs text-muted-foreground">{diagnostic.message}</p>
                    <p className="text-[11px] text-muted-foreground/80">{diagnostic.path.join('.')}</p>
                  </div>
                ))}
              </div>
            )}
